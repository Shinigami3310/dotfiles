...
// Palette singleton. Reads colors.json (generated by matugen on wallpaper change)
// via FileView with watchChanges — the palette updates reactively without restart.
// Fallback colors (dark theme) are used if the file is missing or broken.
QtObject {
    id: root
    ...
    readonly property FileView paletteFile: FileView {
        ...
        function updateColors() {
            try {
                let json = JSON.parse(paletteFile.text);
                root.parsedColors = json.colors;
            } catch (e) {
                // Broken JSON — keep the current palette (or fallback), UI does not crash.
                console.warn("[ThemeColor] Failed to parse colors.json:", e);
            }
        }
        ...
        onFileChanged: {
            // reload() is async: the update happens in onLoadedChanged,
            // when the new content is available. Calling updateColors() here
            // would read stale text.
            paletteFile.reload();
        }
    }
}